...
def getData(uid):
    if not os.path.exists('B站数据/' + str(uid) + '/dynamic.txt'):
        ...
        while 1:
            ...
            endText = etree.HTML(page_text).xpath('//*[@class="end-text"]/text()')
            noText = etree.HTML(page_text).xpath('//*[@class="empty-text tc-slate"]/text()')
...
def dynamicAnalyse(tree):
    ...
    for info in info_list:
        ...
        if dynamicType == 'text p-rel description':
            text = info.xpath('div[3]//*[@class="content-full"]')[0].xpath('string(.)').strip()
            ...
            if forward:
                ...
                if isExit == 'deleted' or isExit == 'deleted-text':
                    ...
                    info_tip = info.xpath('div[3]//*[@class="up-info-tip"]/text()')[0]
                    upName = info.xpath('div[3]/div[2]/div/div[1]/a[2]/text()')[0]  # 转发的 原UPid
                    print('\t\t\t*转发了 * {name} * {info}* '.format(name=upName, info=info_tip), end='')
                    if '图片' in info_tip:
                        forwardText = info.xpath('div[3]/div[2]/div/div[2]/div/div[1]')[0].xpath('string(.)').strip()
                    elif '投稿视频' in info_tip:
                        forwardText = info.xpath('div[3]//*[@class="title"]')[0].xpath('string(.)').strip()
                    elif '文章' in info_tip:
                        forwardText = info.xpath('div[3]//*[@class="title"]')[0].xpath('string(.)').strip()
                    elif '动态' in info_tip:
                        forwardText = info.xpath('div[3]/div[2]/div/div[2]/div/div[1]')[0].xpath('string(.)').strip()
        ...
        # 带图片的动态，或者是投稿视频
            ...
            if isVideo and isVideo[0]:
                videoName = info.xpath('div[3]//*[@class="title"]')[0].xpath('string(.)').strip()
                print('投稿了:', videoName)
                ...
                dressName = info.xpath('div[3]/div[1]/div/div[2]/a/div/div[2]/div[1]/div')[0].xpath(
                    'string(.)').strip()
                text = info.xpath('div[3]/div[1]/div/div[1]/div/div')[0].xpath('string(.)').strip()
                print(text, '* 小卡片 *:', dressName)
            else:
                text = info.xpath('div[3]/div[1]/div/div[1]/div/div')[0].xpath('string(.)').strip()
                emoji = info.xpath('div[3]/div[1]/div/div[1]/div/div/img/@alt')  # b站表情
                outTxt(emoji, text)
